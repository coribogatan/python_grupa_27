...
# cream o func care adauga o prop intr-un dictionar
# obj_param e referinta in memorie a acelui dictionar

def set_hobby(obj_param, key, value):
    obj_param[key] = value
# functia altereaza obj5, chiar daca nu avem un return
obj5 = {"name": "John Wick"}
...
def add_person(param_list, person):
    if len(person) > 2:
        ...
        print("Name needs to be at least 3 characters long!")
...
def add_to_db(database, village, people): #db - database
    database[village] = people.copy() #cu copy nu mai apare Ghita

db = {}
add_to_db(db, "Poienari", lista_persoane)
add_to_db(db, "Gadinti", lista_persoane2)
#fara copy, cand facem append, schimbarea se reflecta si in dictionarul db, pt ca db tine minte referinta spre aceasta lista
lista_persoane2.append("Ghita")

pprint(db) # Ghita apare

var5 = [10]

# var5 tine minte referinta acelei liste in memorie, cand scriu var6 = var5, acum si var 6 are acea referinta
# la aceeasi lista
